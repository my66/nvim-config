...
| Normal | `<Leader>se` | `<C-w>=` | 各分屏平均尺寸 |
| Normal | `<Leader>sx` | `<cmd>close<CR>` | 关闭当前分屏 |
| Normal | `<Leader>sj` | `<cmd>wincmd x<CR>` | 与下一个分屏交换 |
| Normal | `<Leader>to` | `<cmd>tabnew<CR>` | 新建标签页 |
| Normal | `<Leader>tx` | `<cmd>tabclose<CR>` | 关闭当前标签页 |
| Normal | `<Leader>tn` | `<cmd>tabn<CR>` | 下一个标签页 |
| Normal | `<Leader>tp` | `<cmd>tabp<CR>` | 上一个标签页 |
| Normal | `<Leader>tf` | `<cmd>tabnew %<CR>` | 当前文件新标签页打开 |
| Normal | `<C-h>` | `<C-w>h` | 跳到左侧窗口 |
| Normal | `<C-j>` | `<C-w>j` | 跳到下方窗口 |
| Normal | `<C-k>` | `<C-w>k` | 跳到上方窗口 |
| Normal | `<C-l>` | `<C-w>l` | 跳到右侧窗口 |
| Normal | `]d` | `vim.diagnostic.goto_next` | 下一个诊断/错误 |
| Normal | `[d` | `vim.diagnostic.goto_prev` | 上一个诊断/错误 |
...
| Normal | `K` | `vim.lsp.buf.hover` | 悬停文档 |
...
| 模式 | 快捷键 | 命令/函数 | 说明 |
|------|--------|-----------|------|
| Normal | `<Leader>ds` | `vim.diagnostic.open_float` (line) | 查看当前行诊断 |
| Normal | `<Leader>dl` | `vim.diagnostic.open_float` (buffer) | 查看整个缓冲区诊断 |
...
| Normal | `<Leader>fc` | `:Telescope grep_string` | 搜索光标词 |
| Insert (Telescope) | `<C-k>` | 上移选择 | 结果上移 |
| Insert (Telescope) | `<C-j>` | 下移选择 | 结果下移 |
| Insert (Telescope) | `<C-q>` | 发送到 quickfix | 选中项→Quickfix |
...
| Terminal | `jj` / `<Esc>` | `<C-\><C-n>` | 终端→Normal |
...
| Insert | `<Tab>` / `<S-Tab>` | 上下文跳转 | 片段或补全项切换 |
...
> **注意**：本列表基于 `init.lua` 及 `lua/` 目录中显式使用 `vim.keymap.set`, `map`, `noremap` 等 API 定义的按键。若插件内部存在默认快捷键且未被覆盖，则可能未列出。若需完全准确，请结合 `:verbose map` 输出交叉确认。
